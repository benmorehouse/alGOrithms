...
func (this *Heap) TrickleDown(index int){
	...
	for !done{
		if leftChild < len(this.Array) && rightChild < len(this.Array){
			smallestChild := math.Min(float64(this.Array[leftChild].Priority),float64(this.Array[rightChild].Priority))
			if smallestChild < float64(this.Array[index].Priority){
				if smallestChild == float64(this.Array[leftChild].Priority){
